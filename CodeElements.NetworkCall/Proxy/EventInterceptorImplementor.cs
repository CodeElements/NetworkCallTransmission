        ...
        public void ImplementTriggerEvent(TypeBuilder typeBuilder, FieldBuilder[] fieldBuilders, IList<EventInfo> events)
        {
            ...
            for (int i = 0; i < fieldBuilders.Length; i++)
            {
                ...
                if (genericArguments.Length == 1)
                {
                    ...
                    il.Emit(genericArguments[0].GetTypeInfo().IsValueType ? OpCodes.Unbox_Any : OpCodes.Castclass,
                        genericArguments[0]);
                }
                else
                {
                    var emptyField = typeof(EventArgs).GetField("Empty", BindingFlags.Public | BindingFlags.Static);
                    il.Emit(OpCodes.Ldsfld, emptyField);
                }

                il.Emit(OpCodes.Callvirt, eventInfo.EventHandlerType.GetTypeInfo().GetMethod(nameof(EventHandler.Invoke)));
                il.Emit(OpCodes.Ret);
            }
